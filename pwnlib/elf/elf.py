...
from pwnlib import adb, qemu
from pwnlib.asm import *
from pwnlib.context import LocalContext, context
from pwnlib.elf.config import kernel_configuration, parse_kconfig
...
from pwnlib.util import misc, packing
...
class ELF(ELFFile):
    ...
    def __init__(self, path_or_bytes, checksec=True):
        ...
        self.endian = {"ELFDATANONE": "little", "ELFDATA2LSB": "little", "ELFDATA2MSB": "big"}[
    ...
    def _populate_plt(self):
        ...
        with context.local(arch=self.arch, bits=self.bits, endian=self.endian):
            for section in (plt, plt_got, plt_sec, plt_mips):
                ...
                res = emulate_plt_instructions(self, dt_pltgot, section.header.sh_addr, section.data(), inv_symbols)
    ...
    def libc_start_main_return(self):
        ...
        for line in calls:
            ...
            callee_calls = [
                (index, line) for index, line in enumerate(callee_lines) if set(line.split()) & call_instructions
            ]
    ...
    def search(self, needle, writable=False, executable=False):
        ...
        if not segments:
            ...
            for section in super().iter_sections():
                ...
                pagesize = max(pagesize, alignment)
            ...
            for section in super().iter_sections():
                if section.name not in ko_check_segments and not any(
                    section.name.startswith(ko_check_segment) for ko_check_segment in ko_check_segments
                ):
                    continue
    ...
    def checksec(self, banner=True, color=True):
        ...
        res.extend(
            [
                "RELRO:".ljust(12)
                + {"Full": green("Full RELRO"), "Partial": yellow("Partial RELRO"), None: red("No RELRO")}[self.relro],
                "Stack:".ljust(12) + {True: green("Canary found"), False: red("No canary found")}[self.canary],
                "NX:".ljust(12)
                + {
                ...
                "PIE:".ljust(12) + {True: green("PIE enabled"), False: red("No PIE (%#x)" % self.address)}[self.pie],
            ]
        )
        ...
        if len(self.config) > 1:
            ...
            for title, values in config_opts.items():
                ...
                for name, message in sorted(values):
                    line = f"{name} = {red(str(self.config.get(name, None)))}"
                    if message:
                        line += f" ({message})"
                    res.append("    " + line)
    ...
    def set_interpreter(exepath, interpreter_path):
        ...
        try:
            subprocess.check_output(
                ["patchelf", "--set-interpreter", interpreter_path, exepath], stderr=subprocess.STDOUT
            )
        except subprocess.CalledProcessError as e:
