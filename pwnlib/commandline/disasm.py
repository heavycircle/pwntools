...
parser = common.parser_commands.add_parser(
    "disasm", help="Disassemble bytes into text format", description="Disassemble bytes into text format"
)
...
parser.add_argument("-a", "--address", metavar="address", help="Base address", type=str, default="0")


parser.add_argument("--color", help="Color output", action="store_true", default=sys.stdout.isatty())

parser.add_argument("--no-color", help="Disable color output", action="store_false", dest="color")
...
def main(args):
    ...
    if args.color:
        ...
        highlight_bytes = lambda t: "".join(
            map(lambda x: x.replace("00", text.red("00")).replace("0a", text.red("0a")), group(2, t))
        )
        for line in lines:
            if len(line) == 3:
                o, b, i = line
