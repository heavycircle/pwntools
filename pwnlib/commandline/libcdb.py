...
lookup_parser = libc_commands.add_parser(
    "lookup", help="Lookup a libc version by function offsets", description="Lookup a libc version by function offsets"
)
...
lookup_parser.add_argument(
    "-d", "--download-libc", action="store_true", default=False, help="Attempt to download the matching libc.so"
)
...
file_parser = libc_commands.add_parser(
    "file", help="Dump information about a libc binary", description="Dump information about a libc binary"
)
...
file_parser.add_argument(
    "-o", "--offset", metavar="offset", type=str, help="Display all offsets relative to this symbol"
)
...
fetch_parser.add_argument(
    ...
    help="Set libc-database path, If it is empty, the default path will be `context.local_libcdb` (%s)"
...
def get_libc_version(exe):
    res = re.search(rb"libc[ -](\d+\.\d+)", exe.data)
    if res:
    ...
    if args.libc_command == "lookup":
        ...
        for hash_value in args.hash_value:
            ...
            if args.download_libc:
                ...
                shutil.copy(path, f"./libc-{get_libc_version(exe) or Path(path).stem}.so")
        ...
        elif not Path(args.path).exists():
            if yesno(
