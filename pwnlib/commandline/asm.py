...
parser.add_argument(
    "-v", "--avoid", action="append", help="Encode the shellcode to avoid the listed bytes (provided as hex)"
)

parser.add_argument(
    "-n", "--newline", dest="avoid", action="append_const", const="0a", help="Encode the shellcode to avoid newlines"
)

parser.add_argument(
    "-z", "--zero", dest="avoid", action="append_const", const="00", help="Encode the shellcode to avoid NULL bytes"
)


parser.add_argument("-d", "--debug", help="Debug the shellcode with GDB", action="store_true")

parser.add_argument("-e", "--encoder", help="Specific encoder to use")

parser.add_argument("-i", "--infile", help="Specify input file", default=sys.stdin, type=argparse.FileType("r"))

parser.add_argument("-r", "--run", help="Run output", action="store_true")
