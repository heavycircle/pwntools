...
sdk_path = Path(
    "/Library/Developer/CommandLineTools/SDKs/MacOSX15.sdk/System/Library/Frameworks/Kernel.framework/Versions/A/Headers/sys/"
)
...
for key, value in out_data.items():
    ...
    outbuf1_aarch64 += f"{key} = Constant('{key}',{value})\n"
    outbuf2_aarch64 += f"#define {key} {value_octal}\n"

    if key.startswith("SYS_"):
        value = f"{value} + 0x2000000"
        value_octal = f"{value_octal} + 0x2000000"

    outbuf1_amd64 += f"{key} = Constant('{key}',{value})\n"
    outbuf2_amd64 += f"#define {key} {value_octal}\n"
